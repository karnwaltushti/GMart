		...
		<div class="container">
			<div class="row">
				<?php
					...
					$path = chunk_split($pathname, 6, ' ');
					$qry = "SELECT category_id FROM category WHERE category_name = '".$path."'";
					...
					while($row = mysqli_fetch_array($result1)) { ?>
						<div class="col-md-4 text-center text-muted text-warning">
							<?php echo "<img src='uploads/p".$row[4].".jpg' class='img-responsive img-thumbnail' style='height: 300px; width: 360px; padding: 10px 60px 10px 60px;'/>"; ?>
							<form method="post" action="cartUpdate.php">
								<table class="table table-striped table-hover">
									<tr><td>Product Name <td>: <?php echo $row[0]; ?></tr>
									<tr><td>Brand Name <td>: <?php echo $row[1]; ?></tr>
									<tr><td>Quantity <td>: <?php echo $row[2]; ?></tr>
									<tr><td>Price <td>: <?php echo $row[3] . " "; ?><i class="fa fa-inr"></i></tr>
									...
									<tr><td colspan="2">
										<input type="hidden" name="product_id" value="<?php echo $row[4]; ?>"/>
